...
namespace AHeadLib.Net {
    ...
    public class PEFile {
        ...
        public struct IMAGE_SECTION_HEADER {
            ...
            [FieldOffset(36)] uint Characteristics;
        ...
        public static unsafe void analyzeFile(string path, out List<string> exports, out string arch) {
            // Read bytes of DLL
            ...
            IMAGE_DOS_HEADER imageDosHeader =
                CastBytesToType<IMAGE_DOS_HEADER>(sourceFileByteBuffer.Skip(0).Take(sizeof(IMAGE_DOS_HEADER)).ToArray());

            // Get IMAGE_FILE_HEADER to check if the DLL is 32 or 64 bit
            IMAGE_FILE_HEADER imageFileHeader =
                CastBytesToType<IMAGE_FILE_HEADER>(sourceFileByteBuffer.Skip(imageDosHeader.e_lfanew + 4).Take(sizeof(IMAGE_FILE_HEADER)).ToArray());
            ...
            if (imageFileHeader.Machine == 0x8664) // 64bit 0x8664
            {
                ...
                IMAGE_NT_HEADERS64 imageNtHeader =
                    CastBytesToType<IMAGE_NT_HEADERS64>(sourceFileByteBuffer.Skip(imageDosHeader.e_lfanew).Take(sizeof(IMAGE_NT_HEADERS64))
                        .ToArray());
                ...
                for (int i = 0; i < imageNtHeader.FileHeader.NumberOfSections; i++) {
                    ...
                    IMAGE_SECTION_HEADER imageSectionHeader =
                        CastBytesToType<IMAGE_SECTION_HEADER>(
                            sourceFileByteBuffer.Skip(sectionHeadersOffset).Take(SizeOfImageSectionHeader).ToArray());
                    if (imageNtHeader.OptionalHeader.ExportTable.VirtualAddress > imageSectionHeader.VirtualAddress &&
                        imageNtHeader.OptionalHeader.ExportTable.VirtualAddress <
                        (imageSectionHeader.VirtualAddress + imageSectionHeader.VirtualSize)) {
            ...
            } else // 32bit
            {
                ...
                IMAGE_NT_HEADERS32 imageNtHeader =
                    CastBytesToType<IMAGE_NT_HEADERS32>(sourceFileByteBuffer.Skip(imageDosHeader.e_lfanew).Take(sizeof(IMAGE_NT_HEADERS32))
                        .ToArray());
            ...
            IMAGE_EXPORT_DIRECTORY exportTable =
                CastBytesToType<IMAGE_EXPORT_DIRECTORY>(sourceFileByteBuffer.Skip(exportOffset).Take(sizeof(IMAGE_EXPORT_DIRECTORY)).ToArray());
